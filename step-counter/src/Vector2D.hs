module Vector2D (Vector2D, Coord2D, fromList, toList, at, setAt, updateAt, size, Vector2D.map, fill, findCoord) where
...
map f = Vector.map (Vector.map f)

-- searches for value from 2D vector and returns a coordinate
findCoord :: (a -> Bool) -> Vector2D a -> Maybe Coord2D
findCoord fn vect = do
  rowIndex <- Vector.findIndex (isJust . Vector.findIndex fn) vect
  row <- vect !? rowIndex
  colIndex <- Vector.findIndex fn row
